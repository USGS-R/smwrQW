...
The test that compares flipped survival curves can be used for two or more groups and is executied by the \texttt{censKSample.test}. There are two types of the test, ''Peto'' and ''log-rank''; both are described by Helsel (2012). For these data, the ''Peto'' type returns a p-value less than 0.05 and the ''log-rank'' type returns a p-value greater than 0.05, thus giving conflicting conclusions. 
