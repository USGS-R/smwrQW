...
These examples demonstrate some of the functions and statistical methods for comparing paired observations that are available in the \texttt{smwrQW} package. The examples in this vignette use the Atra dataset from the \texttt{NADA} package. The examples in this vignette use the functions \texttt{as.lcens} and \texttt{as.mcens} to convert those data to a form used by the functions demonstrated. The functions demonstrated in these examples will also accept data of class ''qw'' without conversion.
